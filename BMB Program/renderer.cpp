...
{
	return MyDrawEngine::GetInstance()->FillRect(area, _XRGB(31, 31, 26));
}
...
ErrorType Renderer::DrawTextAt(Vector2D position, wchar_t* text,  int colour)
{
  MyDrawEngine::GetInstance()->UseCamera(false);
	 ErrorType Answer = MyDrawEngine::GetInstance()->WriteText(position, text, colour);
...
{
	return MyDrawEngine::GetInstance()->DrawLine(from, to, teamNumber);
}
...
ErrorType Renderer::DrawDot(Vector2D position, int teamNumber)
{
	...
	return MyDrawEngine::GetInstance()->FillRect(dot, teamNumber);
}
